...
def visualize_flow_from_video(video_path, output_video, keep_size=False):
    cap = cv2.VideoCapture(video_path)
    video_writer = cv2.VideoWriter(video_filename, fourcc, fps, (1920, 1080))  # Size should match your output
...
if __name__ == '__main__':
    ...
    parser.add_argument("--video_out", required=False, default='/flow.avi', help="Path to the input video file.")
    parser.add_argument('--seq_dir', default='/mnt/home/C6/Week3/FlowFormerPlusPlus/frames_vdo/frames')
    ...
    video_in = args.video
    video_flow_out = args.video_out

    model = build_model()
    img_pairs = generate_pairs(args.seq_dir, args.start_idx, args.end_idx)
    
    with torch.no_grad():
        visualize_flow_from_video(video_in, video_flow_out)