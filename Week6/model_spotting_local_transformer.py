...
class Model(BaseRGBModel):
    ...
    class Impl(nn.Module):
        def __init__(self, args=None):
            ...
            self.global_tokens = nn.Parameter(torch.randn(1, self.num_global_tokens, self._d))
        ...
        def build_local_global_mask(self, seq_len, num_global, window_size, device):
            total_len = seq_len + num_global
            ...
            # Local attention for each frame
            for i in range(num_global, total_len):
                for j in range(i - window_size, i + window_size + 1):
                    if num_global <= j < total_len:
                        mask[i, j] = True
        ...
        def forward(self, x):
            ...
            # Add global tokens to batch
            global_tokens = self.global_tokens.expand(B, -1, -1)  # [B, G, D]
            tokens = torch.cat([global_tokens, im_feat], dim=1)  # [B, G+T, D]

            # Build local + global attention mask
            attn_mask = self.build_local_global_mask(
                seq_len=T,
                num_global=self.num_global_tokens,
                window_size=4,
                device=im_feat.device
            )  # [G+T, G+T]

            # Apply transformer with mask (inverted for PyTorch)
            tokens = self.transformer(tokens, mask=~attn_mask)

            # Remove global tokens before classification
            frame_tokens = tokens[:, self.num_global_tokens:, :]  # [B, T, D]

            # Classification
            logits = self._fc(frame_tokens)  # [B, T, num_classes+1]
