...
def generate_strategy_A(list_frames, path_json, K=4):
    ...
    train_gt_json_path = generate_gt("gt_train_strategy_A", train_frames, dict_gt)
    eval_gt_json_path = generate_gt("gt_eval_strategy_A", eval_frames, dict_gt)
...
def generate_strategy_B(list_frames, path_json, K=4):
    ...
    for i, partition in enumerate(partitions):
        ...
        train_gt_json_path = generate_gt(f"gt_train_strategy_B_partition_{i}", train_frames, dict_gt)
        eval_gt_json_path = generate_gt(f"gt_eval_strategy_B_partition_{i}", eval_frames, dict_gt)
...
def generate_strategy_C(list_frames, path_json, K=4):
    ...
    for i, partition in enumerate(partitions):
        ...
        tmp_partitions = partitions[:i] + partitions[i+1:]
        eval_frames = set([y for x in (tmp_partitions) for y in x])
        train_gt_json_path = generate_gt(f"gt_train_strategy_C_partition_{i}", train_frames, dict_gt)
        eval_gt_json_path = generate_gt(f"gt_eval_strategy_C_partition_{i}", eval_frames, dict_gt)
        result.append((eval_gt_json_path, eval_frames, train_gt_json_path, train_frames))
