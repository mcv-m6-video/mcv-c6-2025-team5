...
def prepare_image(root_dir, viz_root_dir, fn1, fn2, keep_size):
    ...
    print(f"Shape: {np.array(image1).astype(np.uint8).shape}")
    print(f"Shape: {np.array(image2).astype(np.uint8).shape}")
    ...
    viz_fn2 = osp.join(viz_dir, filename + '.npy')

    return image1, image2, viz_fn, viz_fn2
...
def visualize_flow(root_dir, viz_root_dir, model, img_pairs, keep_size, flow_real=False):
    weights = None
    time = []
    i = 0
    for img_pair in img_pairs:
        ...
        image1, image2, viz_fn, viz_fn2 = prepare_image(root_dir, viz_root_dir, fn1, fn2, keep_size)
        flow, time_it = compute_flow(model, image1, image2, weights)
        time.append(time_it)
        if flow_real:
            np.save(viz_fn2, flow)
        
        flow_img = flow_viz.flow_to_image(flow)
        out_path = osp.join("/mnt/dataout/", f"frame{i:06}.png")
        cv2.imwrite(out_path, flow_img[:, :, [2,1,0]])
        print(f"Save file: {out_path}")
        i = i+1
    time = np.array(time)
    print(f"Mean time: {np.mean(time)}")
    print(f"std: {np.std(time)}")
...
def generate_pairs(dirname, start_idx, end_idx):
    ...
    for idx in range(start_idx, end_idx):
        img1 = osp.join(dirname, f'frame{idx:06}.png')
        img2 = osp.join(dirname, f'frame{idx+1:06}.png')
        # img1 = f'{idx:06}.png'
        # img2 = f'{idx+1:06}.png'
...
def visualize_flow_from_video(video_path):
    ...
    while True:
        ...
        if frame > 0 and previous_frame is not None:
            frame_filename = os.path.join(frames_dir, f"{frame_count:06d}.png")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seq_dir', default='/mnt/datain/')
    parser.add_argument('--start_idx', type=int, default=0)     # starting index of the image sequence
    parser.add_argument('--end_idx', type=int, default=1)    # ending index of the image sequence
    parser.add_argument('--viz_root_dir', default='/mnt/dataout/')
    ...
    print(img_pairs)
    with torch.no_grad():
        visualize_flow(root_dir, viz_root_dir, model, img_pairs, args.keep_size, flow_real=args.save_flow)