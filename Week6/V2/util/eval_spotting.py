...
def evaluate(model, dataset, batch_size=INFERENCE_BATCH_SIZE, nms_window = 5, workers=1):
    ...
    for clip in tqdm(DataLoader(
            dataset, num_workers=workers, pin_memory=True,
        ...
        for i in range(clip['frame'].shape[0]):
            ...
            overlap = clip['overlap'][i].item()
            start_overlap = overlap // 2
            end_overlap = overlap // 2 + overlap % 2
            if start < 0:
            ...
            if start > 0 and start_overlap > 0:
                start += start_overlap
                pred_scores = pred_scores[start_overlap: , :]
            if end < scores.shape[0] and end_overlap > 0:
                end -= end_overlap
                pred_scores = pred_scores[:-end_overlap, :]
